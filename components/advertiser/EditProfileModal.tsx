...
const ImageUploadField: React.FC<{
    ...
    return (
        <div>
            ...
            <div className={`relative group bg-slate-700 ${className}`}>
                {children}
                <label htmlFor={id} className="absolute inset-0 bg-black/60 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity rounded-full">
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Edit Profile">
            <form onSubmit={handleSubmit} className="space-y-6">
                 <div>
                    <label htmlFor="email" className="block text-sm font-medium text-slate-300">Email (Login)</label>
                    <input
                        type="email"
                        id="email"
                        value={user.email}
                        readOnly
                        className="mt-1 block w-full bg-slate-900/50 border border-slate-700 rounded-md shadow-sm py-2 px-3 text-slate-400 cursor-not-allowed"
                        aria-describedby="email-description"
                    />
                    <p id="email-description" className="mt-2 text-xs text-slate-500">Your email is used for logging in and cannot be changed.</p>
                </div>

                <div className="flex items-center gap-6">
                     <ImageUploadField label="Logo" onFileChange={handleLogoChange} className="h-24 w-24 rounded-full flex-shrink-0">
                         <img src={logoPreview || user.logoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.username)}&background=3b82f6&color=fff&size=128`} alt="Company Logo" className="h-full w-full object-cover rounded-full" />
                    </ImageUploadField>
                    <div className="flex-grow">
                        <label htmlFor="username" className="block text-sm font-medium text-slate-300">Company Name</label>
                        <input
                            type="text"
                            id="username"
                            value={username}
                ...
                <div>
                    ...
                    <div className="relative group bg-slate-700 h-40 rounded-lg">
                        {bannerPreview || user.bannerUrl ? (
                         ...
                         <label htmlFor="banner-upload" className="absolute inset-0 bg-black/60 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity rounded-lg">
