...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, title, subtitle }) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-50 flex justify-center items-center p-4 transition-all duration-300 ease-out ${isOpen ? 'bg-black bg-opacity-70' : 'bg-opacity-0'}`}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div 
        className={`bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all duration-300 ease-out flex flex-col max-h-[90vh] ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} 
